...
<div class="customers-form">
	...
	<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        <h4 class="modal-title" id="itemModalLabel"><i class="fa fa-table"></i> พนักงานขับรถ</h4>
    </div>
    ...
    $.post(
        ...
        if(result.status == 'success') {
            ...
            if(result.action == 'create') {
                //$(\$form).trigger('reset');
                $(document).find('#modal-customer').modal('hide');
                $.pjax.reload({container:'#customers-grid-pjax'});
